...
function renderStats() {
  ...
  // Aggregate monitoring data from all devices
  ...
  devList.forEach(d => {
    ...
    if (d.brightness_pct) {
      totalBrightness += d.brightness_pct;
      brightCount++;
    }
  });
  ...
  grid.innerHTML = `
    ...
    <div class="stat-card">
      ...
      <div class="stat-value ${alerts.length > 0 ? 'color-danger' : 'color-success'}">${alerts.length}</div>
    </div>
  `;
}
...
function renderDevicePanel(dev) {
  ...
  const panelId = `panel-${dev.device_id}`;

  return `
    <div class="device-panel ${connected ? '' : 'offline'}" id="${panelId}">
      <div class="device-header" onclick="toggleDevice('${dev.device_id}')">
        ...
        <div style="flex:1">
          ...
          <div class="device-meta">${dev.ip} • Port ${dev.port || 5200} • ${dev.firmware_version || '—'}</div>
        </div>
        ...
        <button class="btn btn-sm btn-danger" onclick="event.stopPropagation();removeDevice('${dev.device_id}')">✕</button>
      ...
      <div class="device-body" id="body-${dev.device_id}">
          ...
          <div>
            <div class="section" style="margin-bottom:10px;">
              ...
              <div class="info-grid">
                ...
                ${infoItem('Cards', lm.card_count || '—')}
                ${infoItem('RC Firmware', lm.firmware || '—')}
                ${infoItem('MAC', lm.mac_address || '—')}
              </div>
            </div>
            <div class="section" style="margin-bottom:10px;">
              <div class="section-title">🖥 Device</div>
              <div class="info-grid">
                ${infoItem('Model', di.model_code || si.device_type || '—')}
                ${infoItem('Serial', di.serial || '—')}
                ${infoItem('Build', si.build_date || '—')}
                ${infoItem('Ports', si.ethernet_ports || '—')}
                ${infoItem('Inputs', si.input_count || '—')}
                ${infoItem('Port 2', dev.port2_active ? 'Active' : 'Standby')}
              </div>
            </div>
          </div>
          <div>
        ...
        ${cards.length > 0 ? `
        <div class="section">
          <div class="section-title">📦 Receiving Cards</div>
          <div class="card-grid">
            ${cards.map(c => `
...
function renderAlerts() {
  ...
  if (alerts.length === 0) {
    log.innerHTML = '<p class="color-muted" style="padding:20px;text-align:center;">No errors recorded</p>';
    return;
  }
  ...
  log.innerHTML = alerts.map(a => {
    ...
    return `
      <div class="error-entry">
        ...
        <div style="flex:1">
          <div class="error-device">${a.device || '?'}</div>
